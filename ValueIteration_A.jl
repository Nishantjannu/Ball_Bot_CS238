...
# ╔═╡ 4e152d10-85c8-4829-a06d-dbbb25cee86c
# using DiscreteValueIteration

# ╔═╡ 482cc9f0-7342-4ba5-95a4-795fe94a860a
# solver = ValueIterationSolver();

# ╔═╡ aae7f57c-1d2a-4ad4-8ced-403e68cc6217
# π  = solve(solver, mdp);
...
begin
	...
	null_state = State(-1,-1) # terminal state
	...
	# Defining the Dict and calling it 
	...
	function T(s, a) # Transition function
		...
		for (i, a′) in enumerate(𝒜)
			prob = (a′ == a) ? p_tran : (1 - p_tran) / (Nₐ - 1)
			# change if needed 
			destination = apply(s, a′) # returns x,y of next state
			# next_states[1] = old state
	...
	function R(s, a=missing) # Reward function
		...
		elseif s == State(-1,-1)
			return 0
		elseif s in [State(i,j) for i in 4:5 for j in 3:8]
			return -50
		end
		return -5
	end
	...
	function POMDPs.observation(pomdp::DroneSurveillancePOMDP{PerfectCam}, a::Int64, s::DSState)
		...
		# takes care of checking if obervation is in fov
	...
	function render(mdp, s=nothing;
		...
		terminal_states = filter(s->reward(mdp, s) != -5, states(mdp))
		...
		# return title!("Return to Waiting Station: Reward States")
		return title!("Stochastic: Finite Horizon")
	...
	function render_new(mdp, s=nothing;
			values=nothing, policy=nothing,  show_grid=true, label_rewards=true,
			cmap=ColorScheme([RGB(0.7, 0, 0), RGB(1, 1, 1), RGB(0, 0.4, 0)]))

		# color scheme is from red,white,green gradient
		
		gr() # start animation
	
		s_valid = setdiff(states(mdp), [null_state])

		# basically only second part i.e map(s-> value(values,s), s_valid)
		# value(values, s) is from DiscreteValueIteration 
		U = map(s->isnothing(values) ? reward(mdp,s) : value(values,s), s_valid)
		# π = map(s->isnothing(values) ? reward(mdp,s) : value(values,s), s_valid)
		# rebuild U
	
		xmax = ymax = Int(sqrt(length(U)))
		Uxy = reshape(U, xmax, ymax)
		# convert array into matrix for plot
		heatmap(Uxy',leg=:none,ratio=:equal,frame=:box,tickor=:out,c=cmap.colors)
		...
		# filters out non-terminal states
		...
		for sT in s_valid
			...
			annotate!([(sT.x, sT.y, (b, :black, :c, 10, "Computer Modern"))])
		end
		#end
		...
		# return title!("Return to Waiting Station Policy")
		return title!("Stocastic: Finite Horizon (time crunch)")
	end
...
# ╔═╡ d9eae2ba-28f1-4a64-aba1-f47fd9a38059
π[s]
# value(π, s)

# ╔═╡ c7d683a8-a3f6-42a7-aa48-12f33f0ba3df
render_new(mdp; values=U, policy = π)
