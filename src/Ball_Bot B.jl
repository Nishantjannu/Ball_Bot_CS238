...
@with_kw mutable struct BallBot{M} <: POMDP{DSState, Int64, Int64} #@with_kw allows assigning default values inside the struct
    ...
    region_A::DSPos = [6,4] # [rand(1:10),rand(1:10)]
    region_B::DSPos = [1,10]  # rand([[1,1],[8,2],[1,8],[8,7]])
...
function POMDPs.reward(pomdp::BallBot, s::DSState, a::Int64)
    if s.bot == s.player 
        return -100
    elseif (s.bot == [1,1])||(s.bot == [1,8])
        return 100 #20
    elseif (s.bot == [8,2])||(s.bot == [8,7])
        return 50
    end
    return -5
end
